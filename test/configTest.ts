...
interface ConfigObject {
  modules: {
    [index: string]: {
      [index: string]: { replacer: string; finder: string; extender: string };
    };
  };
      ...
      Object.keys(oJsonObj.modules).forEach(function(sKey: string) {
        ...
        Object.keys(oModule).forEach(function(sModuleKey: string) {
          ...
          if (sReplacer) {
            ...
            assert.ok(sReplacerFile, 'Replacer ' + sReplacer + ' exists');
      ...
      Object.keys(oJsonObj.modules).forEach(function(sKey: string) {
        ...
        Object.keys(oModule).forEach(function(sModuleKey: string) {
          ...
          if (sFinder) {
            ...
            const sNormalizedFinderFile = path.normalize(jsDir + sFinderFile);
            assert.ok(
          ...
          if (sExtender) {
            ...
            assert.ok(sExtenderFile, 'Finder ' + sExtender + ' exists');
