...
const aFileArray =
	[ path.join("test", "myfile2.js"), path.join("test", ".myfile12.js") ];
...
describe("FileFinder", function() {
	it("getAllFileInfos", function() {
		const oStub = sinon.stub(FileFinder.prototype, "getFiles")
						  .returns(Promise.resolve(aFileArray));
		return new FileFinder(oBuilder).getFileInfoArray().then(function(
			aAllModules) {
			oStub.restore();
			...
			assert.deepEqual(
				...
				aFileArray, "should contain 2 entries");
		});
	});

	it("findByPath", function() {
		const oStubGetFiles = sinon.stub(FileFinder.prototype, "getFiles")
								  .returns(Promise.resolve(aFileArray));
		const oStub = sinon.stub(FileInfo.prototype, "loadContent")
