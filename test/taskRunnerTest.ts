...
import {CustomFileFinder, CustomLocalFileInfo, CustomMetaReporter, CustomMigrationTask, CustomReporter} from "./util/testUtils";
...
describe("taskRunner", function() {
	it("should run tasks and modify files one after the other", function() {
		...
		const aFileInfos = [];
		const customLocalFileInfo1 =
			new CustomLocalFileInfo(rootDir, "file1.js");

		customLocalFileInfo1.addOnSaveListener(function() {
			fileSaved.push("file1");
		});
		const customLocalFileInfo2 =
			new CustomLocalFileInfo(rootDir, "file2.js");
		...
		return taskRunner
			.processModules(
				aTasks, aFileInfos, customReporter, customFileFinder, "", null,
				false)
			.then(function() {
				assert.deepEqual(
					fileSaved, [ "file1", "file2" ],
					"files should be saved in correct order");
				assert.deepEqual(
					fileMigrated,
					[
						"task1:file1", "task2:file1", "task1:file2",
						"task2:file2"
					],
	...
	it("should run migrate right after migrate before executing the next task",
	   function() {
		   ...
		   return taskRunner
			   .processModules(
				   aTasks, aFileInfos, customReporter, customFileFinder, "",
				   null, false)
			   .then(function() {
				   assert.deepEqual(
					   fileMigrated,
					   [
						   "analyze task1", "migrate task1", "analyze task2",
						   "migrate task2"
					   ],
					   "files should be migrated in correct order");
			   });
	...
	it("should run post task after main task", function() {
		...
		const expectedContent =
			fs.readFileSync(rootDir + "jquery0.expected.js", "utf8");
