import {AnalyzeCharacter, CodeStyleAnalyzer} from "../../src/util/CodeStyleAnalyzer";
...
describe("CodeStyleAnalyzerTest", function() {
	it("analyze strings ui5 style", function() {
		...
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.INDENT), true,
			"should be tab");
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.NEWLINE), "N",
			"should be N");
	});
	...
	it("analyze strings runtime style", function() {
		...
		assert.strictEqual(
			...
			"should be tab");
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.NEWLINE), "RN",
			"should be N");
	});
	...
	it("analyze strings runtime style spaces 2 EOL RN", function() {
		...
		const analyzer = new CodeStyleAnalyzer(source);
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.INDENT), 2,
			"should be 2 spaces");
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.NEWLINE), "RN",
			"should be RN");
	...
	it("analyze strings runtime style spaces 4 EOL N", function() {
		...
		const analyzer = new CodeStyleAnalyzer(source);
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.INDENT), 4,
			"should be 4 spaces");
		assert.strictEqual(
			analyzer.getMostCommon(AnalyzeCharacter.NEWLINE), "N",
			"should be N");
