sap.ui.define([
...
], function (FragmentDialog, CONST, JSONModel, TableColumn, Text, ganttEntity,//eslint-disable-line max-params valueHelpBuilder,
	Filter, FilterOperator, FilterType, FilterBar, SearchField, ValueHelpDialog, ganttFilter, compLibrary, jQuery, capitalize) {
	"use strict";
	...
	var FilterProgramCampaignDialog = FragmentDialog.extend(CONST.namespace + ".control.dialog.FilterProgramCampaignDialog", {
		...
		_addValueHelpDialogColumn: function (oTable, sField) {
			var sCapitalizedField = capitalize(sField);
		...
		_buildValueHelpDialog: function () {
			if (!this._oValueHelpDialog) {
				var oValueHelpDialog = new ValueHelpDialog("filterProgramCampaignValueHelp", {
					...
					tokenDisplayBehaviour: DisplayBehaviour.descriptionOnly
				});
		...
		onValueHelpDialogUpdate: function () {
			...
			setTimeout(this._oValueHelpDialog["update"].bind(this._oValueHelpDialog), 1);
		},
