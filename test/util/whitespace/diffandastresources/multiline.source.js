sap.ui.define([
...
], function(themingParameters, constants, NumberFormat, DateFormat, CurrencyFormat, ValueState) {
	"use strict";
	...
	var _formatter = {
		...
		getMediaTypeMicroChartTitle: function(sProgramTypeProperty) {
			...
			return jQuery.sap.formatMessage(sI18nPatternForTitle, sLocalizedLabelForProposedSpend);
		},
		...
		formatBudgetPath: function(oPlanCoordinate, sPlanningHierarchy, oMetaModel) {
			...
			try {
				...
				jQuery.sap.log.error("Error parsing the planning model's planning hierarchy");
			}
		...
		formatObjectSubtitle: function(dValidFrom, dValidTo, sMktAreaDesc) {
			...
			return sDateInterval ? jQuery.sap.formatMessage("{0}, {1}", sDateInterval, sMktAreaDesc) : sMktAreaDesc;
		},
	...
	jQuery.sap.declare("hpa.cei.amp.program.util.formatter");
	hpa.cei.amp.program.util.formatter = _formatter;
