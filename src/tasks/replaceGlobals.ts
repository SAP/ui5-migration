...
function getDefineCall(
  ...
  const defineCalls = ASTUtils.findCalls(oAst, SapUiDefineCall.isValidRootPath);
  ...
  visitor.visit(oAST, {
    visitMemberExpression(oPath) {
      if (isGlobalContext(oPath.value)) {
        ...
        if (result) {
          const wouldReplace = wouldReplaceCall(moduleMap, result, defineCall);
...
async function analyse(args: Mod.AnalyseArguments): Promise<{}> {
  ...
  // return
  const defineCall = getDefineCall(ast, args.file.getFileName(), args.reporter);
  if (!defineCall) {
    ...
    if (oConfig.modules.hasOwnProperty(sKeyOldModule)) {
      ...
      for (const sOldImportName in oldImports) {
        ...
        if (oldImports.hasOwnProperty(sOldImportName)) {
          ...
          if (!replacerName) {
            replacerName = oldImport.functionName ? 'Module' : 'ModuleFunction';
          }
          ...
          if (oldImport.newModulePath) {
            if (oldImport.newModulePath in mImportRequireNames) {
              ...
              if (p < 0) {
                ...
                mImportRequireNames[oldImport.newModulePath] = newImportName;
              } else {
                // reuse import
  ...
  for (const oCallPath of aCallsToReplace) {
    ...
    if (oResult) {
      ...
      if (oResult.import.newModule && oResult.import.hasToBeRequired) {
        ...
        if (p < 0) {
          analysis.newRequires.push({
            ...
            origRequireName: calculateImportName(oResult.import.config),
          });
        }
      }
      ...
      if (oResult.import.oldModule !== GLOBALS) {
        const p = analysis.removeRequires.indexOf(oResult.import.oldModule);
        if (p < 0) {
...
async function migrate(args: Mod.MigrateArguments): Promise<boolean> {
  ...
  for (const oReplace of analyseResult.replaceCalls) {
    ...
    try {
      ...
      const oResult = mReplacerFuncs[oReplace.import.replacerName].replace(
        oNodePath,
      ...
      if (
        ...
        oNodePath.parentPath.value.property.type === Syntax.Identifier &&
        oNodePath.parentPath.value.property.name
      ) {
        args.reporter.collect(oNodePath.parentPath.value.property.name, 1);
      }
      ...
      const sComment =
        ...
        (args.config.comments && args.config.comments.failedReplacementComment);
      if (sComment) {
  ...
  for (const oRequire of newRequires) {
    const bWillBeRemoved = aRequireNames.indexOf(oRequire.origRequireName) > -1;
...
const replaceGlobals: Mod.Task = {
  description: 'Replaces usage of global jQuery functions with module imports',
