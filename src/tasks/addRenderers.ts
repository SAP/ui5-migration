    ...
    if (stmt.type === Syntax.VariableDeclaration) {
      stmt.declarations.forEach(decl => {
        if (
          ...
          typeof (decl.init.arguments[0] as ESTree.Literal).value === 'string'
        ) {
      ...
      if (stmt.argument && stmt.argument.type === Syntax.Identifier) {
        ...
        if (
          ...
          rendererDefinedResult.classNode = classDefinitions[exportName];

          rendererDefinedResult.moduleName = classNames[exportName];

          classDefinitions[exportName].properties.forEach(function(prop) {
...
async function analyse(
  ...
  if (
    ...
    args.reporter.storeFinding('Missing Renderer', defineCall.node.callee.loc);
    return {
      defineCall,
