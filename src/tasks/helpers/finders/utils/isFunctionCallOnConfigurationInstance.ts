...
import isFunctionCallOnCore from "./isFunctionCallOnCoreInstance";
...
export default function (node: ESTree.Node, functionName: string): boolean {
	return (
		...
		((node.callee.object.type === Syntax.Identifier &&
			node.callee.object.name.toLowerCase().includes("configuration")) ||
			isFunctionCallOnCore(node.callee.object, "getConfiguration"))
	);
}
