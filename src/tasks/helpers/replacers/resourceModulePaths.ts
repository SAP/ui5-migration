...
const replaceable: ASTReplaceable = {
  replace(
    ...
    const isGetModulePathCall = oldModuleCall === 'jQuery.sap.getModulePath';
    ...
    function appendSuffixIfNecessary(
      ...
      } else if (aModuleArgs[1] && aModuleArgs[1].type === Syntax.Literal) {
        return builders.binaryExpression(
          '+',
          oExpressionWithoutSuffix,
    ...
    if (aArgs[0] && aArgs[0].type === Syntax.Literal) {
      ...
      if (aArgs.length === 1 && sResourceValue !== '') {
        ...
        const aMatches = rOrginalSubTypes.exec(aSegments[aSegments.length - 1]);
        if (aMatches) {
          sSuffix = aMatches[0];
      ...
      if (isGetModulePathCall) {
        if (bIgnoreExoticCases) {
          sResourceCode = '(' + sResourceCode + ').replace(/\\./g, "/")';
        } else {
          sResourceCode =
      ...
      if (aArgs.length === 1) {
        let sReplacement;
        if (bIgnoreExoticCases) {
          sReplacement = 'sap.ui.require.toUrl(' + sResourceCode + ')';
        } else {
