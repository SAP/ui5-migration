 ...
 * - oCore.getCurrentFocusedControlId()
 *
 * With
 *   FocusHandler.getCurrentFocusedControlId()
 *
 *
...
const replaceable: ASTReplaceable = {
	replace(
		...
		const oFocusHandlerIdent: ESTree.Identifier = builders.identifier(
		...
		const oNodeModule: ESTree.Expression = builders.callExpression(
			builders.memberExpression(
				oFocusHandlerIdent,
				builders.identifier("getCurrentFocusedControlId")
			),
			[]
		);
