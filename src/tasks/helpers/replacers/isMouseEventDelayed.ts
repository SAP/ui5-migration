...
const replaceable: ASTReplaceable = {
	...
	replace(
		...
		void {
			...
			const oNodeCall =
				builders.callExpression(builders.identifier(name), []);

			if (oInsertionPoint.type ===
				Syntax.IfStatement) {  // if(jQuery.sap.isMouseEventDelayed)
				oInsertionPoint.test = oNodeCall;
			} else if (
				oInsertionPoint.type ===
				Syntax.VariableDeclarator) {  // var test =
											  // jQuery.sap.isMouseEventDelayed
				oInsertionPoint.init = oNodeCall;
			} else if (
				oInsertionPoint.type === Syntax.AssignmentExpression &&
				node.name ===
					"right") {  // test = jQuery.sap.isMouseEventDelayed and
								// NOT jQuery.sap.isMouseEventDelayed = test
				oInsertionPoint.right = oNodeCall;
			} else if (
				oInsertionPoint.type ===
				Syntax.LogicalExpression) {  // test &&
											 // jQuery.sap.isMouseEventDelayed
											 // || test2
