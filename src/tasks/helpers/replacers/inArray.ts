    ...
    if (
      ...
      if (aArray.type === Syntax.ArrayExpression) {
        const oReplacement = builders.callExpression(
          builders.memberExpression(aArray, builders.identifier('indexOf')),
        ...
        if (oInsertionPoint.type === Syntax.BinaryExpression) {
          ...
          const oConditionalExpression = builders.conditionalExpression(
            aArray,
            builders.callExpression(oNodeIndexOfCall, oInsertion.arguments),
            builders.unaryExpression('-', builders.literal(1), true)
          );

          oInsertionPoint[node.parentPath.name] = oConditionalExpression;
        }
      }
    } else {
      throw new Error(
