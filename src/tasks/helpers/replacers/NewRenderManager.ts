...
const replaceable: ASTReplaceable = {
	replace(
		...
		const identName = name || config.newVariableName;
		const oInsertionPoint = node.parentPath.value;

		const oNodeModule: ESTree.Expression = builders.newExpression(
			builders.identifier(identName),
			[]
		);
