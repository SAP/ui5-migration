...
export async function getSupportedTasks(): Promise<MigrationTask[]> {
  ...
  for (const sFileName of aFileNames) {
    if (sFileName.endsWith('js')) {
      const module = require(path.join(sTaskPath, sFileName)) as MigrationTask;
...
export async function processModules(
  ...
  for (const oFileInfo of aFileInfo) {
    ...
    oReporter.setContext(
      Object.assign(oReporter.getContext(), { fileName: oFileInfo.getPath() })
    );
    ...
    for (const oTask of aTasks) {
      ...
      oAnalyseResult = await oTask
        ...
        .catch(function(err) {
          ...
          taskReporter.report(Mod.ReportLevel.DEBUG, `Stack: ${err.stack}`);
        });
