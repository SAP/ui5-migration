...
export class ConsoleReporter extends BaseReporter {
  ...
  report(level: ReportLevel, msg: string, loc?: ESTree.SourceLocation) {
    ...
    if (this.getContext().fileName) {
      ...
      if (loc) {
        ...
        if (oLocation.start) {
          sOutFileName +=
            ':' + oLocation.start.line + ':' + oLocation.start.column;
        } else {
          sOutFileName += ':1:1';
        }
      } else {
  ...
  private static log(level: string, sMessage: string, aParams: string[] = []) {
    switch (level) {
  ...
  reportCollected(level: ReportLevel) {
    ...
    const sReporter = this.getContext().logPrefix || this.getContext().taskName;
    this.setContext({ logPrefix: '', fileName: '' });
    ConsoleReporter.log(level, '');
    ConsoleReporter.log(level, 'Report for \x1b[31m%s\x1b[0m:', [sReporter]);
    const that = this;
    this.oMap.forEach(function(value, key, map) {
      if (typeof value === 'number') {
        that.report(level, key + ': ' + value);
      } else {
        that.report(level, 'value: ' + key + ': entries: ' + value.length);
      }
    });
    this.oMap.clear();
  }
