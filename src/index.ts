...
class MetaJSONReporter extends MetaConsoleReporter {
  ...
  reportCollected(level: ReportLevel) {
    this.getReporters().forEach(oReporter => oReporter.reportCollected(level));
  }
}
        ...
        logResult = oReporterResults
          .filter(oReport => {
            return (
              ...
              oReport.reports.filter(fnCodeReplacementFilter).length > 0
            );
          })
...
export async function migrate(oArgs: IndexArgs): Promise<{}> {
  ...
  if (Array.isArray(aExcludedPaths)) {
    aExcludedPaths.forEach(function(sExcludePath) {
      ...
      aFilterPromises.push(
        FsFilterFactory.createFilter(sExcludePath).then(function(oFilter) {
  ...
  if (oArgs.ignoreFile && (await FileUtils.isFile(oArgs.ignoreFile))) {
    ...
    aFilterPromises.push(
      FsFilterFactory.createIgnoreFileFilter(oArgs.ignoreFile).then(function(
        oFilter
      ) {
        builder.excludeFilesFromFilter([oFilter]);
      })
  ...
  if (Array.isArray(aIncludedPaths)) {
    aIncludedPaths.forEach(function(sIncludePath) {
      ...
      aFilterPromises.push(
        FsFilterFactory.createFilter(sIncludePath).then(function(oFilter) {
  ...
  if (!aIncludedPaths || aIncludedPaths.length === 0) {
    oTaskRunnerReporter.report(ReportLevel.INFO, 'Searching current folder');
    aFilterPromises.push(
