...
export class SapUiDefineCall {
  ...
  constructor(
    ...
    if (
      ...
      ((this.dependencyArray &&
        this.factory.params.length > this.dependencyArray.elements.length) ||
  ...
  modifyDependency(iIndex: number, sModule: string, sShortcut?: string) {
    if (!this.dependencyArray) {
      ...
      this.dependencyArray = this.node.arguments[0] as ESTree.ArrayExpression;
    }
  ...
  addDependency(sModule: string, sShortcut?: string) {
    ...
    let i = this.dependencyArray.elements.findIndex(
      oElement => oElement.type === 'Literal' && oElement.value === sModule
    );
    if (i >= 0) {
    ...
    if (aAlreadyMatchingElements.length > 1 && sParam) {
      ...
      if (iIndex > 0) {
        const matchingIndexElements = aAlreadyMatchingElements.filter(
          oMatchingElement => {
            return (
              this.dependencyArray.elements.indexOf(oMatchingElement) === iIndex
            );
          }
        );
  ...
  getNodeOfImport(sModule: string): ESTree.Literal {
    ...
    const aAlreadyMatchingElements = this.dependencyArray.elements.filter(
      function(oElement) {
        if (oElement.type !== 'Literal') {
          throw new Error('Dependency is not a literal in ' + that.name);
        }
        return oElement.value === sModule;
      }
    );
  ...
  _analyzeBody() {
      ...
      if (stmt.type === Syntax.VariableDeclaration) {
        stmt.declarations.forEach(decl => {
          if (decl.id.type === Syntax.Identifier) {
            ...
            if (shortcut) {
              const globalName =
                shortcut.module.replace(/\//g, '.').replace(/\.library$/, '') +
                '.' +
    ...
    this.factory.body.body.forEach(stmt => {
      if (
        ...
        this.exportName &&
        SapUiDefineCallUtils.isMemberOf(stmt.expression.left, this.exportName)
      ) {
        if (
          stmt.expression.left.type === Syntax.MemberExpression &&
          stmt.expression.left.object.type === Syntax.MemberExpression &&
          stmt.expression.left.object.property.type === Syntax.Identifier &&
          ...
          this.reporter.report(
            ReportLevel.DEBUG,
            '  found class method: ' +
              SapUiDefineCallUtils.getObjectName(stmt.expression.left)
          );
          Object.defineProperty(stmt.expression.right, '__classMethodName', {
            enumerable: false,
