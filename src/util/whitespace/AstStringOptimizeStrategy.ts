...
export class AstStringOptimizeStrategy implements StringOptimizeStrategy {
  ...
  private static keepPrecedingWhitespace(
    ...
    if (targetIndent !== modifiedIndent) {
      ...
      AstStringOptimizeStrategy.log(reporter, `AST: remove`, modifiedIndent);
  ...
  private static keepSucceedingWhitespace(
    ...
    if (targetOutdent !== modifiedOutdent) {
      ...
      AstStringOptimizeStrategy.log(reporter, `AST: remove`, modifiedOutdent);
  ...
  private iterateAstNode(
    ...
    if (typeof oSourceNode === 'string' && typeof oModifiedNode === 'string') {
      return;
    }
    const oOptions = {
      modified: false,
    ...
    if (Array.isArray(oModifiedNode) && Array.isArray(oSourceNode)) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (
          AstStringOptimizeStrategy.isNullOrUndefined(oCurrentModifiedNode) ||
          AstStringOptimizeStrategy.isNullOrUndefined(oCurrentSourceNode)
        ) {
  ...
  private static modifyContent(
    ...
    if (direction === PROCESS_DIRECTION.PRECEDING) {
      for (let i = whitespaceToRemove.length - 1; i >= 0; i--) {
        ...
        const iIndexToDelete = iIndex - (whitespaceToRemove.length - 1 - i);
    ...
    if (
      ...
      StringWhitespaceUtils.isWhitespace(aOptimizedContent[lastDeletedIndex])
    ) {
  ...
  private static hasFunctionCall(aSapUiDefineArguments) {
    ...
    if (
      ...
      return (
        beforeLastArgument && beforeLastArgument.type === 'FunctionExpression'
      );
    }
      ...
      if (
        ...
        if (Array.isArray(oChild) || oChild.type) {
          if (AstStringOptimizeStrategy.isUseStrict(oChild)) {
            ...
            oUseStrict = AstStringOptimizeStrategy.findUseStrict(oChild);
          }
        }
      }
    });
  ...
  async hasStructuralChange(
    ...
    this.iterateAstNode(
      ...
      (oSourceNode, oModifiedNode, oOptions) => {
        if (
          AstStringOptimizeStrategy.isSapUiDefineCall(oModifiedNode) &&
          (AstStringOptimizeStrategy.isSapUiDefineCall(oSourceNode) ||
            AstStringOptimizeStrategy.isIIFEWithoutArgs(oSourceNode))
        ) {
          result = false;
          // break iteration
          return false;
  ...
  async optimizeString(original: string, modified: string): Promise<string> {
    ...
    const onDiff = function(oSourceNode, oModifiedNode, oOptions) {
      ...
      if (!oOptions.added && !oOptions.valueChange && !oOptions.arrayMod) {
        return;
      }
      ...
      if (
          ...
          oSourceNode.slice(1).forEach(oCurrentNode => {
            ...
            if (currentModifiedIndent !== lastTargetIndent) {
              ...
              styles.push({
                ...
                lineBreakIndex: oSourceNode.indexOf(oCurrentNode),
        ...
        oModifiedNode.slice(1).forEach(oCurrentNode => {
          ...
          if (styles.length > 0) {
            const currentNodeIndex = oModifiedNode.indexOf(oCurrentNode);
