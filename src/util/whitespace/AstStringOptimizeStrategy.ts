...
export class AstStringOptimizeStrategy implements StringOptimizeStrategy {
	...
	private static keepPrecedingWhitespace(
		aOptimizedContent: string[], targetIndent: string,
		modifiedIndent: string, oModifiedNodeElement, reporter?: Reporter) {
		if (targetIndent !== modifiedIndent) {
			AstStringOptimizeStrategy.log(
				reporter, `AST: whitespace diff for preceding element`);
			AstStringOptimizeStrategy.log(
				reporter, `AST: remove`, modifiedIndent);
			AstStringOptimizeStrategy.log(reporter, `AST: add`, targetIndent);
			AstStringOptimizeStrategy.log(
				reporter, `AST: index: ${oModifiedNodeElement.range[0] - 1}`);
			if (reporter) {
				reporter.collect("AstStringOptimizeStrategy.preceding", 1);
			}
			AstStringOptimizeStrategy.modifyContent(
				aOptimizedContent, oModifiedNodeElement.range[0] - 1,
				modifiedIndent, targetIndent, PROCESS_DIRECTION.PRECEDING);
		}
	}

	private static keepSucceedingWhitespace(
		aOptimizedContent: string[], targetOutdent: string,
		modifiedOutdent: string, oModifiedNodeElement, reporter?: Reporter) {
		if (targetOutdent !== modifiedOutdent) {
			AstStringOptimizeStrategy.log(
				reporter, `AST: whitespace diff for succeeding element`);
			AstStringOptimizeStrategy.log(
				reporter, `AST: remove`, modifiedOutdent);
			AstStringOptimizeStrategy.log(reporter, `AST: add`, targetOutdent);
			AstStringOptimizeStrategy.log(
				reporter, `AST: index: ${oModifiedNodeElement.range[1]}`);
			if (reporter) {
				reporter.collect("AstStringOptimizeStrategy.succeeding", 1);
			}
			AstStringOptimizeStrategy.modifyContent(
				aOptimizedContent, oModifiedNodeElement.range[1],
				modifiedOutdent, targetOutdent, PROCESS_DIRECTION.SUCCEEDING);
		}
	}
	...
	private iterateAstNode(
		oSourceNode, oModifiedNode, onDiff, oParentModifiedNode?) {
		if (typeof oSourceNode === "string" &&
		...
		if (Array.isArray(oModifiedNode) && Array.isArray(oSourceNode)) {
			...
			for (let i = 0; i < length; i++) {
				...
				if (AstStringOptimizeStrategy.isNullOrUndefined(
						oCurrentModifiedNode) ||
					AstStringOptimizeStrategy.isNullOrUndefined(
						oCurrentSourceNode)) {
					oOptions.arrayMod = true;
					oOptions.modified = true;
				} else if (AstStringOptimizeStrategy.valuesDiffer(
							   oCurrentModifiedNode, oCurrentSourceNode)) {
					oOptions.arrayMod = true;
		...
		Object.keys(oSourceNode).forEach(function(sSourceNodeKey) {
			if (!filterAstAttributes.includes(sSourceNodeKey) &&
				oSourceNode[sSourceNodeKey] && oModifiedNode[sSourceNodeKey]) {
				that.iterateAstNode(
					oSourceNode[sSourceNodeKey], oModifiedNode[sSourceNodeKey],
					onDiff, oModifiedNode);
			}
		});
	}
	...
	private static modifyContent(
		aOptimizedContent, iIndex: number, whitespaceToRemove: string,
		whitespaceToAdd: string, direction: PROCESS_DIRECTION) {
		...
		if (!aOptimizedContent[lastDeletedIndex] ||
	...
	private static hasFunctionCall(aSapUiDefineArguments) {
		...
		if (aSapUiDefineArguments.length > 1 &&
	...
	private static isSapUiDefineCall(node) {
		return (
			...
			node.expression.arguments.length > 0 &&
			AstStringOptimizeStrategy.hasFunctionCall(
	...
	private static findUseStrict(oParsedModified) {
		...
		Object.keys(oParsedModified).forEach((sChildKey) => {
			if (!oUseStrict && !filterAstAttributes.includes(sChildKey) &&
	...
	async hasStructuralChange(original: string, modified: string):
		Promise<boolean> {
		...
		let result = true;
		this.iterateAstNode(
			oParsedOriginal, oParsedModified,
			(oSourceNode, oModifiedNode, oOptions) => {
				if (AstStringOptimizeStrategy.isSapUiDefineCall(
						oModifiedNode) &&
	...
	async optimizeString(original: string, modified: string): Promise<string> {
		if (this.reporter) {
			this.reporter.report(
				ReportLevel.TRACE, `Performing AstStringOptimizeStrategy`);
		}
		...
		if (oUseStrictModifiedNode) {
			const useStrictPrec =
				AstStringOptimizeStrategy.getPrecedingNodeWhitespaces(
					modified, oUseStrictModifiedNode);
			let targetIndent;
			...
			if (oUseStrictOrigNode) {
				targetIndent =
					AstStringOptimizeStrategy.getPrecedingNodeWhitespaces(
						original, oUseStrictOrigNode);
			} else {
				const useStrictSucc =
					AstStringOptimizeStrategy.getSucceedingNodeWhitespaces(
						modified, oUseStrictModifiedNode);
				targetIndent = AstStringOptimizeStrategy.getEOL(useStrictSucc) +
					AstStringOptimizeStrategy.getIndent(useStrictSucc);
			}
			AstStringOptimizeStrategy.keepPrecedingWhitespace(
				aOptimizedContent, targetIndent, useStrictPrec,
				oUseStrictModifiedNode, that.reporter);
		}
		...
		const onDiff = function(oSourceNode, oModifiedNode, oOptions) {
			...
			if (!oOptions.added && !oOptions.valueChange &&
				!oOptions.arrayMod) {
				return;
			}
			...
			if (oOptions.parent &&
				(oOptions.parent.type === "BlockStatement")) {
				return;
			}
			...
			if (Array.isArray(oModifiedNode) && Array.isArray(oSourceNode) &&
				oModifiedNode.length > 0 && oSourceNode.length > 0) {
				...
				let lastTargetIndent =
					AstStringOptimizeStrategy.getPrecedingNodeWhitespaces(
						original, oSourceNode[oSourceNode.length - 1]);
					...
					oSourceNode.slice(1).forEach((oCurrentNode) => {
						const currentModifiedIndent =
							AstStringOptimizeStrategy
								.getPrecedingNodeWhitespaces(
									original, oCurrentNode);
						if (currentModifiedIndent !== lastTargetIndent) {
							const previousNode =
								oSourceNode[oSourceNode.indexOf(oCurrentNode) - 1];
							styles.push({
								lineLength : AstStringOptimizeStrategy
				...
				const firstModifiedIndent =
					AstStringOptimizeStrategy.getPrecedingNodeWhitespaces(
						modified, oModifiedNode[0]);
				AstStringOptimizeStrategy.keepPrecedingWhitespace(
					aOptimizedContent, firstTargetIndent, firstModifiedIndent,
					oModifiedNode[0], that.reporter);
				...
				oModifiedNode.slice(1).forEach((oCurrentNode) => {
					...
					const currentModifiedIndent =
						AstStringOptimizeStrategy.getPrecedingNodeWhitespaces(
							modified, oCurrentNode);
					AstStringOptimizeStrategy.keepPrecedingWhitespace(
						aOptimizedContent, targetIndent, currentModifiedIndent,
						oCurrentNode, that.reporter);
				});

				// outdent each but the last node
				oModifiedNode.slice(0, oModifiedNode.length - 1)
					.forEach((oCurrentNode) => {
						const currentModifiedOutdent =
							AstStringOptimizeStrategy
								.getSucceedingNodeWhitespaces(
									modified, oCurrentNode);
						AstStringOptimizeStrategy.keepSucceedingWhitespace(
							aOptimizedContent, firstTargetOutdent,
							currentModifiedOutdent, oCurrentNode,
							that.reporter);
					});
				...
				const lastModifiedOutdent =
					AstStringOptimizeStrategy.getSucceedingNodeWhitespaces(
						modified, oModifiedNode[oModifiedNode.length - 1]);
				AstStringOptimizeStrategy.keepSucceedingWhitespace(
					aOptimizedContent, lastTargetOutdent, lastModifiedOutdent,
					oModifiedNode[oModifiedNode.length - 1], that.reporter);
			}
