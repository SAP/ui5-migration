...
import {AstStringOptimizeStrategy, NodeFilter} from "./AstStringOptimizeStrategy";
import {DiffStringOptimizeStrategy} from "./DiffStringOptimizeStrategy";

export class DiffAndAstStringOptimizeStrategy implements
	StringOptimizeStrategy {
	...
	async optimizeString(original: string, modified: string): Promise<string> {
		const astStringOptimizeStrategy =
			new AstStringOptimizeStrategy(this.reporter);
		...
		if (!await astStringOptimizeStrategy.hasStructuralChange(
				original, optimized)) {
			const diffOptimization =
				new DiffStringOptimizeStrategy(this.reporter);
			optimized =
				await diffOptimization.optimizeString(original, optimized);
			const changedLines =
				diffOptimization.getChangedRanges(original, optimized);
