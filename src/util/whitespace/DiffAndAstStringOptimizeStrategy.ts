...
export class DiffAndAstStringOptimizeStrategy
  ...
  async optimizeString(original: string, modified: string): Promise<string> {
    ...
    if (
      ...
      const diffOptimization = new DiffStringOptimizeStrategy(this.reporter);
      optimized = await diffOptimization.optimizeString(original, optimized);
      ...
      const wasModifiedNodeFilter: NodeFilter = {
        isValid(node) {
          if (node.range) {
            ...
            return changedLines.some(nodeRange.isOverlappedBy.bind(nodeRange));
          }
          return true;
        },
      };
