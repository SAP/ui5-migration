...
async function getModuleDefinedInLibrary(name: string, apiInfo: APIInfo) {
  return apiInfo.getSymbol(name).then(function(oApiSymbol) {
    ...
    if (oSymbol.module && oSymbol.export && /\/library/.test(oSymbol.module)) {
      return oSymbol;
    }
  ...
  defineCall.dependencyArray.elements.slice().forEach(function(depObj) {
    if (depObj.type === Syntax.Literal) {
      ...
      const pAddPromise = getModuleDefinedInLibrary(
        ...
        reporter.collect('Added library dependency: ' + oSymbol.module, 1);
        ...
        if (modify) {
          ...
          let localRef = defineCall.getParamNameByImport(oSymbol.module);
          if (!localRef) {
            ...
            defineCall.addDependency(oSymbol.module, sLibraryParamName);
            localRef = defineCall.getParamNameByImport(oSymbol.module);
          }
          ...
          const variableDeclaration = builders.variableDeclaration('var', [
