    ...
    visitMemberExpression(path) {
      if (that.isStaticGlobal(path, reporter)) {
        ...
        if (that.isRHSOfInstanceOf(path)) {
          reporter.report(
            ReportLevel.DEBUG,
            '  RHS of instanceof operator not touched in pass 1: ' + name + '.',
            path.value.loc
          );
          return false;
        }
...
function replaceSelfCalls(
  ...
  if (declareResult.path) {
    if (modify) {
      ...
      oAnalysisResult['remove-path'] = oAnalysisResult['remove-path'] || [];
        ...
        if (that.isStaticGlobal(path, reporter)) {
          ...
          if (
            ...
            if (ModuleNameComparator.compare(name, sDeclareName)) {
              ...
              path.parentPath.parentPath.parent.node[
                path.parentPath.parentPath.parent.name
              ][path.parentPath.parentPath.name] = newVariableDeclaration;
      ...
      recast.visit(ast, {
        visitCallExpression(path) {
          if (
            ...
            path.parentPath.value.type !== Syntax.VariableDeclarator &&
            path.value.arguments[0].value.toString() === sDeclareName
          ) {
            const sVariableName = VariableNameCreator.getUniqueVariableName(
              aUsedVariables,
              sDeclareName
        ...
        visitMemberExpression(path) {
          if (that.isStaticGlobal(path, reporter)) {
            ...
            if (ModuleNameComparator.compare(name, sDeclareName)) {
              ...
              path.parent.node[path.name] = builders.identifier(sVariableName);
              bFileWasModified = true;
...
function visitCode(
      ...
      if (that.isJQuerySAPRequireCall(path.value)) {
        ...
        if (modify) {
          ...
          if (!processResult.pruned) {
            ...
            reporter.storeFinding('found jQuery.sap.require', path.loc);
            oAnalysisResult['remove-path'] =
              oAnalysisResult['remove-path'] || [];
            oAnalysisResult['remove-path'].push('jQuery.sap.require');
          }
        }
      }
      this.traverse(path);
        ...
        if (oSymbol) {
          ...
          if (isSafe || !config.onlySafeReplacements) {
            ...
            if (modify) {
              ...
              bFileWasModified =
                bFileWasModified || addDependencyResult.modified;
            } else {
            ...
            if (localRef == null) {
              reporter.report(
                ReportLevel.TRACE,
                "keep global access to '" + name + "' (reference to self)",
                path.value.loc
              );
            } else if (oSymbol.export === undefined) {
              reporter.report(
                ReportLevel.TRACE,
                "keep global access to '" + name + "' (no module export known)",
                path.value.loc
              );
            } else {
              let replacement;
              ...
              if (oSymbol.export) {
                ...
                if (modify) {
                  ...
                  bFileWasModified =
                    bFileWasModified || oAddShortCutResult.modified;
                  ...
                  reporter.storeFinding('add shortcut', path.value.loc);
                ...
                if (
                  path.parent &&
                  path.parent.node.type === Syntax.VariableDeclarator &&
                  path.parent.node.init === path.node
                ) {
                  if (
                    path.parent.node.id.type === Syntax.Identifier &&
              ...
              if (oSymbol.member) {
                ...
                const identifier = builders.identifier(oSymbol.member);
                replacement = builders.memberExpression(
                  replacement,
                  identifier
                );
                replacementStr = replacementStr + '.' + oSymbol.member;
              }
              reporter.report(
                ReportLevel.DEBUG,
              ...
              if (!isSafe && config.addTodoForUnsafeReplacements) {
                replacement.comments = replacement.comments || [];
              ...
              if (modify) {
                ...
                oAnalysisResult['replace'] = oAnalysisResult['replace'] || [];

                oAnalysisResult['replace'].push({ path, replacement });
              }
            }
            return false;
          } else {
  ...
  recast.visit(ast, {
    visitMemberExpression(path) {
      if (that.isStaticGlobal(path, reporter) && that.isRHSOfInstanceOf(path)) {
        const name = that.getObjectName(path.value);
        // try to find a module
        const oSymbol = fnGetSymbol(name);
        ...
        // use that
        const localRef =
          oSymbol && oSymbol.module && that.findDependency(oSymbol.module);
        if (localRef) {
          reporter.report(
            ReportLevel.DEBUG,
            "  replace '... instanceof " +
          ...
          if (modify) {
            ...
            oAnalysisResult['replace'].push({ identifier: localRef, path });
          }
          return false;
        }
          ...
          if (modify) {
            path.parent.replace(
              builders.callExpression(
                ...
                [path.parent.node.left, builders.literal(oSymbol.module)]
              )
            );
            bFileWasModified = true;
            reporter.report(
...
module.exports = {
  ...
  async ui52amd(
    ...
    if (defineCalls.length > 1) {
      ...
      reporter.report(ReportLevel.DEBUG, 'found define call at position 0');
    } else {
      ...
      const relevantTopLevelStatements = this.getRelevantNodes(topLevelStmts);
      if (
        ...
        block = (((relevantTopLevelStatements[0] as ESTree.ExpressionStatement)
          ...
          .callee as ESTree.FunctionExpression).body as ESTree.BlockStatement;
    ...
    if (blockStatementOfCreatedDefineCall) {
      if (modify) {
        ...
        bFileWasModified = bFileWasModified || oConvertExportResult.modified;
        ...
        reporter.storeFinding('Convert export', (ast as ESTree.Program).loc);
    ...
    const encapsulate = function(aPromiseResults) {
      return function(name: string) {
        const aRes = aPromiseResults.filter(function(oResult) {
          return oResult && oResult.symbol && oResult.symbol.name === name;
        });
        if (aRes.length > 0) {
    ...
    return fillApiInfo(ast, that, reporter, config, apiInfo)
      ...
      .then(function(oObject) {
        bFileWasModified = bFileWasModified || oObject.oResult.fileWasModified;
        const aModificationsPromise: Array<Promise<{}>> = [];
        ...
        return Promise.all(aModificationsPromise).then(function() {
          ...
          if (oObject.oResult.declareName && oObject.oResult.bWasDefined) {
            ...
            if (
              ...
              if (modify) {
                aBody.push(
                  builders.returnStatement(builders.identifier(sVariableName))
                );
                reporter.report(
                  Mod.ReportLevel.DEBUG,
                  'Added return statement of ' + sVariableName,
                  oLastStatement.loc
                );
              } else {
                reporter.storeFinding('return statement', oLastStatement.loc);
            ...
            if (!that.defineCall.bExportsNode) {
              if (modify) {
                ...
                oAnalysisResult['globalExport'].push({ module: 'true' });
              }
            }
          }
  ...
  normalizeDependencyQuotes(defineCall: SapUiDefineCall, reporter: Reporter) {
    ...
    if (
      ...
      reporter.report(ReportLevel.DEBUG, '  normalizing dependency quotes');
      ...
      if (
        ...
        iCountDoubleQuotes !== defineCall.dependencyArray.elements.length
      ) {
        ...
        const elements: Expression[] = defineCall.dependencyArray.elements.map(
          (dep: ESTree.Expression) => {
            if (
              ...
              dir.value = quote + (dep as ESTree.Literal).value + quote;
              return dir;
            }
            return dep;
          }
  ...
  convertExport(
    ...
    function handleExtendCall(call) {
      ...
      if (className.replace(/\./g, '/') === defineCall.name) {
        ...
        mainClassNameShortcut = className.slice(className.lastIndexOf('.') + 1);
      }
    }
    ...
    if (node && node.type === Syntax.BlockStatement) {
      ...
      (node as ESTree.BlockStatement).body.forEach((stmt, idx) => {
        if (stmt.type === Syntax.ExpressionStatement) {
          const expression = (stmt as ESTree.ExpressionStatement).expression;
          if (
            ...
            (node as ESTree.BlockStatement).body[
              ...
              builders.variableDeclarator(
                ...
                // that name is
                // not used yet
                (expression as ESTree.AssignmentExpression).right
              ),
            ]);
            bModified = true;
      ...
      if (exportVar) {
        (node as ESTree.BlockStatement).body.push(
          builders.returnStatement(builders.identifier(mainClassNameShortcut))
        );
  ...
  addDependency(
    ...
    if (!this.dependencies[moduleName]) {
      ...
      if (
        ...
        defineCall.dependencyArray.elements.forEach((mod: ESTree.Node, idx) => {
          if (
            paramName &&
            /^jquery\.sap\./.test((mod as ESTree.Literal).value.toString()) &&
            idx < defineCall.paramNames.length
          ) {
            paramName = null;
            this.dependencies[moduleName].local = defineCall.paramNames[idx];
            reporter.report(
              ReportLevel.DEBUG,
              ' using ' +
  ...
  async resolveSymbol(name, apiInfo) {
    ...
    return apiInfo.getSymbol(name).then(function(symbol) {
      ...
      if (symbol) {
        ...
        // be excluded
        else if (symbol.member && symbol.member.module === 'ui5loader') {
          return undefined;
        }
  ...
  isSafeModule(moduleName, config) {
    if (this.defineCall) {
      ...
      if (
        ...
        this.defineCall.getAbsoluteDependencyPaths().indexOf(moduleName) >= 0
      ) {
        return true;
      }
    }
    if (config.basicModules && config.basicModules.indexOf(moduleName) >= 0) {
  ...
  isJQuerySAPDeclareCall(node: ESTree.Node) {
    const bMemberCall =
      ...
      (node as ESTree.CallExpression).callee.type === Syntax.MemberExpression &&
      ((node as ESTree.CallExpression).callee as ESTree.MemberExpression).object
        .type === Syntax.MemberExpression &&
      (((node as ESTree.CallExpression).callee as ESTree.MemberExpression)
        .object as ESTree.MemberExpression).object.type === Syntax.Identifier;
    if (bMemberCall) {
      ...
      if (bJQuery) {
        const call = (member.object as ESTree.MemberExpression).property;
        return (
  ...
  isExtendCall(node: ESTree.Node) {
    return (
      ...
      typeof ((node as ESTree.CallExpression).arguments[0] as ESTree.Literal)
        .value === 'string'
    );
  },
  ...
  isIIFE(node: ESTree.Node) {
    if (
      ...
      ((node as ESTree.ExpressionStatement).expression as ESTree.CallExpression)
...
function identifyUnusedImports(amdCleanerUtilInstance) {
  ...
  recast.visit(amdCleanerUtilInstance.defineCall.factory.body.body, {
    visitIdentifier(path) {
      ...
      if (
        (parent.type === Syntax.MemberExpression && node === parent.object) ||
        ...
        // the NAME
        (parent.type === Syntax.LabeledStatement && node !== parent.label) ||
        (parent.type === Syntax.ContinueStatement && node !== parent.label) ||
        (parent.type === Syntax.BreakStatement && node !== parent.label) ||
  ...
  aUnusedParamNames.forEach((unusedParamName: string) => {
    ...
    if (
      ...
      if (modify) {
        ...
        reporter.report(Mod.ReportLevel.DEBUG, 'Remove dependency', ast.loc);
      } else {
