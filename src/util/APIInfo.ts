...
export class APIInfo {
  ...
  async findLibrary(sEntityName: string): Promise<string> {
    ...
    if (this.oApiVersion && Object.keys(this.oApiVersion).length > 0) {
      ...
      return this.loadJSON(this.rootPath, 'resources/sap-ui-version.json').then(
        function(oObject: {}) {
          that.oApiVersion = oObject as VersionInfo;
          return APIInfo.findLibraryName(sEntityName, that.oApiVersion);
        },
        function() {
          return 'sap.ui.core';
        }
  ...
  postProcessAPIJSON(oLibDoc: LibDoc, reporter: Reporter): LibDoc {
      ...
      if (props) {
        for (j = 0; j < props.length; j++) {
          if (props[j].static && props[j].module) {
            modules[props[j].module] = modules[props[j].module] || [];
      ...
      if (meths) {
        for (j = 0; j < meths.length; j++) {
          if (meths[j].static && meths[j].module) {
            modules[meths[j].module] = modules[meths[j].module] || [];
    ...
    function guessExport(defaultExport: string, symbol: LibDocSymbol) {
      if (symbol.name === defaultExport) {
        ...
        reporter.report(
          ReportLevel.TRACE,
          '    **** could not identify module export for API ' + symbol.name
        );
      }
    }
          ...
          if (symbols[i].properties) {
            for (j = 0; j < symbols[i].properties.length; j++) {
              if (
                ...
                sEntityName ===
                  symbols[i].name + '.' + symbols[i].properties[j].name
              ) {
                return {
                  symbol: symbols[i],
          ...
          if (symbols[i].methods) {
            for (j = 0; j < symbols[i].methods.length; j++) {
              if (
                ...
                sEntityName ===
                  symbols[i].name + '.' + symbols[i].methods[j].name
              ) {
                return {
                  symbol: symbols[i],
  ...
  async getSymbol(sEntityName: string): Promise<LibDocSymbol> {
    ...
    result = this.findLibrary(sEntityName).then(function(sLibrary) {
      return that.getLibraryMetaInformation(sLibrary).then(function(oLibDoc) {
    ...
    return this.getSymbol(sEntityName).then(function(symbol: LibDocSymbol) {
      ...
      if (meta) {
        ...
        if (resolveInheritance && symbol.extends) {
          return that.getMeta(symbol.extends, true).then(function(meta2) {
            if (meta2) {
              meta.defaultAggregation =
                meta.defaultAggregation || meta2.defaultAggregation;
  ...
  getLocalResourcesAPIJSON(sLibrary: string) {
    return this.mApiIncludedResources && this.mApiIncludedResources[sLibrary];
  }
        ...
        oResources.resources.forEach(function(resource) {
          if (
            ...
            oLibraryDoc.symbols.push({
              kind: 'namespace',
              name: resource.module.replace(/\.js$/, '').replace(/\//g, '.'),
    ...
    try {
        ...
        aLoadLibraryApiJson.push(
          this.loadJSON(
            ...
            function() {
              ...
              that.oLibraryDocumentation[sLibrary] = EMPTY_LIB_DOC;
      ...
      if (oResources) {
        ...
        aLoadLibraryApiJson.push(
          this.loadJSON(
            this.rootPath,
            'resources/' + sLibrary.replace(/\./g, '/') + '/resources.json'
          ).catch(function() {
            that.reporter.report(
              ReportLevel.TRACE,
