...
function _flattenTask(oTask: MigrationTask): MigrationTask[] {
	...
	if (oTask.postTasks) {
		...
		const aPostTasks =
			oTask.postTasks.map((sKeyword: string): MigrationTask => {
 ...
 */
export async function flattenTaskArray(aTasks: MigrationTask[]):
	...
	// is finally returned
	return aTasks.map(_flattenTask)
		.reduce(
			(aFlattenTasks: MigrationTask[], aCurrentTasks: MigrationTask[]):
				MigrationTask[] => {
