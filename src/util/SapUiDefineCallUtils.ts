...
export function isDirective(node: ESTree.Node) {
  return (
    ...
    (node as ESTree.ExpressionStatement).expression.type === Syntax.Literal &&
...
export function getObjectName(node: ESTree.Node): string | undefined {
  if (node.type === Syntax.MemberExpression) {
    ...
    if (oMemberExpression.property.type === Syntax.Identifier) {
      ...
      return getObjectName(oMemberExpression.object) + '.' + oProperty.name;
    }
...
export function checkForShortcutExpression(
  ...
  while (
    ...
    (leftmost as ESTree.MemberExpression).property.type === Syntax.Identifier
  ) {
    propertyPath.unshift(
      ((leftmost as ESTree.MemberExpression).property as ESTree.Identifier).name
    );
