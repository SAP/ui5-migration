...
export async function start(): Promise<void> {
  ...
  const argsBuilder = yargs
    ...
    .example('$0 migrate "src"', 'Migrates all js files in the "src" folder')
    .command('analyze', 'Analyzes all js files in the current folder')
    .example('$0 analyze "src"', 'Analyzes all js files in the "src" folder')
    .help()
    .version()
    .option('exclude-path', {
      alias: ['e'],
    ...
    .option('output-format', {
      ...
      default: path.join(__dirname, '../../defaultConfig/printer.config.json'),
      normalize: true,
    })
    .option('task', {
      alias: ['t'],
    ...
    .check(function(argv) {
      const bValid =
        argv._.length >= 1 && ['migrate', 'analyze'].includes(argv._[0]);
